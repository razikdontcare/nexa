...
import { startWebServer, setConnectionStatus, setLatestQR, attachSocket } from './web/server.js'
...
async function start() {
  ...
  sock.ev.on('connection.update', async (u) => {
    ...
    if (connection === 'close') {
      ...
      if (shouldReconnect) start().catch(err => logger.error(err, 'Reconnect failed'))
    } else if (connection === 'open') {
      logger.info('WhatsApp connection opened')
      setConnectionStatus('open')
      metrics.setStatus('open')
      try {
        const me = (sock as any)?.authState?.creds?.me?.id || (sock as any)?.user?.id
